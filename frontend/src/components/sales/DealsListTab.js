...
export default function DealsListTab({ onChanged }) {
  ...
  const act = async (deal) => {
    setBusyId(deal.id);
    try {
      await api.post(`/deals/${deal.id}/book`, {});
      toast.success("Deal dikonfirmasi (booked).");
      refresh();
        ...
        <div className="flex flex-wrap justify-end gap-1.5" onClick={(e) => e.stopPropagation()}>
          ...
          {["reserved", "booked"].includes(d.status) ? (
            <Button data-testid={DEALS.cancelBtn} size="sm" variant="ghost"
              className="text-rose-600" disabled={busyId === d.id} onClick={() => setCancelFor(d)}>
  ...
  return (
    <>
      ...
      <CancelDealDialog deal={cancelFor} open={!!cancelFor}
        onOpenChange={(v) => !v && setCancelFor(null)} onDone={refresh} />
    </>
  );
}
