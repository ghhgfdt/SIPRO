...
import { Handshake, UserPlus, Ban } from "lucide-react";
...
import CancelDealDialog from "@/components/sales/CancelDealDialog";
...
import { LEADS, P53, DEAL_CANCEL } from "@/constants/testIds";
...
export default function LeadUnitsTab({
  ...
  return (
    <div className="space-y-3">
          ...
          {deals.map((d) => (
            <div key={d.id} data-testid="lead-deal-row" data-deal={d.id}
              ...
              <div className="flex w-full flex-wrap items-center gap-2 border-t pt-2">
                ...
                {mayBook && ["reserved", "booked"].includes(d.status) ? (
                  <Button data-testid={DEAL_CANCEL.btn} size="sm" variant="ghost"
                    className="text-rose-600 hover:bg-rose-50 hover:text-rose-700"
                    onClick={() => setCancelFor(d)}>
                    <Ban className="mr-1.5 h-4 w-4" /> Batalkan Reservasi
                  </Button>
                ) : null}
                {d.status === "reserved" ? (
                  <p className="text-xs text-muted-foreground">
      ...
      <CancelDealDialog deal={cancelFor} open={!!cancelFor}
        onOpenChange={(v) => !v && setCancelFor(null)} onDone={onChanged} />
    </div>
  );
}
